                        ...
                        <form action="{{ route('categories.update', $category->id) }}" method="post">
                            ...
                            <div class="input mb-4">
                                ...
                                <select name="status" id="status" class="form-select">
                                    ...
                                    <option {{ $category->status == App\Enum\StatusType::ACTIVE ? 'selected' : '' }}
                                        value="{{ App\Enum\StatusType::ACTIVE }}">Active</option>
                                    <option {{ $category->status == App\Enum\StatusType::INACTIVE ? 'selected' : '' }}
                                        value="{{ App\Enum\StatusType::INACTIVE }}">Inactive</option>
                                </select>
                                @error('status')
                                    <span class="position-absolute text-danger"><small>{{ $message }}</small></span>
                                @enderror
